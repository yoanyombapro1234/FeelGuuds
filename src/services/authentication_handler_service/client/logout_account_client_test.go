...
func TestAuthenticationHandlerServiceClient_LogoutAccount(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"log out non-existent account scenario",
			email,
			password,
		...
		{
			"log out account with misconfigured account id",
			"",
			password,
			nil,
			true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if !tt.accountExists {
				err = c.LogoutAccount(context.Background(), NON_EXISTENT_ACCOUNT_ID)
			} else {
				err = c.LogoutAccount(context.Background(), id)
			}
