...
func TestAuthenticationHandlerServiceClient_UnLockAccount(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"unlock non-existent account scenario",
			email,
			password,
		...
		{
			"unlnock account that already exists and isnt locked scenario",
			email,
			password,
		...
		{
			"unlock account that already exists and is unlocked scenario",
			"",
			password,
			nil,
			true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if !tt.accountExists {
				err = c.UnlockAccount(context.Background(), NON_EXISTENT_ACCOUNT_ID)
			} else {
				if tt.shouldAccountBeUnLockPrematurely {
					_ = c.UnlockAccount(context.Background(), id)
				}

				err = c.UnlockAccount(context.Background(), id)
			}
